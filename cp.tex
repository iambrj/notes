...
\begin{itemize}
    \item\url{https://cses.fi/problemset/task/1662}
    \item A subarray $A[l, r]$ is divisible by $n$ if $prefix[0, r] = prefix[0,
        l - 1]$ modulo $n$, where $prefix[0, i]$ is the prefix sum from 0 to
        $i$. Thus, it suffices to compute, for each reminder $r$ in $[0, n -
        1]$, frequency of prefix sums with that reminder and then find
...
\begin{itemize}
    ...
    \item Idea: if a subarray $A[i,j]$ has at most $k$ distinct values, then
        ...
        two pointers + sliding window by exploiting above invariant.
\end{itemize}
\end{document}
