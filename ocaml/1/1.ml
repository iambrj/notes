...
printf "Second langauge is %s\n" languages.(1);; (* Accessing array element *)
languages.(1) <- "Coq";; (* updating array element *)
printf "Second langauge is %s\n" languages.(1);;
...
let x = {contents = 1};; (* ref type, for to simulate pointers. Record with single variant *)
printf "x has %d\n" !x;; (* you can dereference a pointer! *)
x := 5;; (* you can update a pointer's contents! *)
printf "x now has %d\n" !x;;
